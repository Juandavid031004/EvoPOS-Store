...
export const Login = ({ onLogin }: LoginProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Convert username to uppercase and email to lowercase
    const upperUsername = username.toUpperCase();
    const lowerEmail = email.toLowerCase();

    // Check if email is authorized
    if (!authorizedEmails.includes(lowerEmail)) {
      toast.error('Este correo no está autorizado para acceder al sistema');
      return;
    }

    // Get stored users for this business
    const storedUsers = JSON.parse(localStorage.getItem(`users_${lowerEmail}`) || '[]');

    // Find user by email and username
    const user = storedUsers.find((u: any) => 
      u.email.toLowerCase() === lowerEmail && 
      u.username === upperUsername
    );

    // If default admin and doesn't exist in localStorage for this business
    if (upperUsername === defaultAdmin.username && !user) {
      if (password === defaultAdmin.password) {
        // Create admin in localStorage for this business
        const adminUser = {
          id: '1',
          email: lowerEmail,
          ...
          rol: 'admin',
          sucursal: '1',
          permisos: DEFAULT_PERMISSIONS.admin,
          activo: true
        };
        localStorage.setItem(`users_${lowerEmail}`, JSON.stringify([adminUser]));
        ...
        localStorage.setItem(`businessData_${lowerEmail}`, JSON.stringify(initialData));

        onLogin(lowerEmail, upperUsername, password);
        toast.success('Bienvenido Administrador - Nuevo Negocio');
      } else {
    ...
    // If existing user, verify credentials
    if (user) {
      if (!user.activo) {
        toast.error('Usuario inactivo. Contacte al administrador');
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-indigo-900 via-purple-900 to-pink-900 flex items-center justify-center py-6 px-4 sm:px-6 lg:px-8 animate-gradient-x relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-20 -left-20 w-60 h-60 bg-pink-500/30 rounded-full mix-blend-soft-light filter blur-3xl animate-float"></div>
        <div className="absolute -bottom-20 -right-20 w-60 h-60 bg-purple-500/30 rounded-full mix-blend-soft-light filter blur-3xl animate-float" style={{ animationDelay: '2s' }}></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-60 h-60 bg-indigo-500/30 rounded-full mix-blend-soft-light filter blur-3xl animate-float" style={{ animationDelay: '4s' }}></div>
      </div>
