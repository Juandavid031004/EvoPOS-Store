...
export const Dashboard = ({ products, sales, expenses, config, users }: DashboardProps) => {
  ...
  const getTotalCost = (salesList: Sale[]) => {
    return salesList.reduce((sum, sale) => {
      return sum + sale.productos.reduce((prodSum, item) => {
        ...
        return prodSum + (product ? product.costo * item.cantidad : 0);
      }, 0);
    }, 0);
  };
  ...
  const lowStockProducts = products.filter(p => p.stock <= config.stockMinimo);
  ...
  const stats = [
    ...
    },
    {
      ...
      description: `${lowStockProducts.length} productos debajo de ${config.stockMinimo} unidades`
    }
  ];
                    ...
                    <tr key={product.id} className="group hover:bg-gradient-to-r hover:from-gray-50 hover:to-slate-50 transition-colors duration-300">
                      ...
                      <td className="py-4">
                        <span className={`
                          ...
                          transition-all duration-300 group-hover:scale-105
                          ${product.stock <= config.stockMinimo / 2 
                      ...
                      <td className="py-4">
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 text-sm font-medium rounded-full bg-rose-100 text-rose-700 group-hover:bg-rose-200 transition-colors duration-300">
